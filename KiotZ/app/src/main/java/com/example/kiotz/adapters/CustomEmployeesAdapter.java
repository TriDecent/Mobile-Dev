...
public class CustomEmployeesAdapter extends RecyclerView.Adapter<CustomEmployeesAdapter.MyViewHolder> {
    private final Context context;
    private final List<Employee> employees;
    ArrayList<EmployeeTotalAmountSold> employeeTotalAmountSold;
    public CustomEmployeesAdapter(Context context, List<Employee> employees, ArrayList<EmployeeTotalAmountSold> employeeTotalAmountSold) {
        this.context = context;
        this.employees = employees;
        this.employeeTotalAmountSold = employeeTotalAmountSold;
    }

    @NonNull
    @Override
    public CustomEmployeesAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        var inflater = LayoutInflater.from(context);
        var view = inflater.inflate(R.layout.item_employee_view, parent, false);
        return new CustomEmployeesAdapter.MyViewHolder(view);
    }
        ...
        holder.cvEmployee.setOnClickListener(v -> {
            ...
            if (position != RecyclerView.NO_POSITION) {
                for (EmployeeTotalAmountSold i: employeeTotalAmountSold) {
                    if (i.getEmployeeID().equals(employees.get(position).ID()))
                        Toast.makeText(context, "Employee sold " + String.valueOf(i.getTotalAmount()) + " in total", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
